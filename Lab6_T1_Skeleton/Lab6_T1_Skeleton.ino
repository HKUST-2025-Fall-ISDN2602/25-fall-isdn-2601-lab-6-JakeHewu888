// L298N Driver Pin
#define MOTOR_ENA 33   // Use a PWM-capable pin
#define MOTOR_IN1 19
#define MOTOR_IN2 5
...
double getPosition() {
  ...
  const float countsPerRevolution = 338.0; // <-- Replace this after observation
  position = float(encoderCount) * 360.0 / countsPerRevolution;
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (command == "F") {
      ...
      digitalWrite(MOTOR_IN2, LOW);
      analogWrite(MOTOR_ENA, 0); // Set motor speed
    } else if (command == "B") {
      // Backward direction
      digitalWrite(MOTOR_IN1, LOW);
